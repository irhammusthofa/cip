...
class Aksi extends User_Controller
{
    var $_kode7 = ['AC1','AC2','AC3'];
    var $_kode8 = ['AC4','AC5'];
    ...
    public function simpan($kode,$id_cip){
        ...
        if ($q){
			...
			redirect('user/aksi/edit/'.base64_encode($kode).'/'.base64_encode($id_cip));
	...
	{
        if($index==7) $param['kode'] = $this->_kode7;
        if($index==8) $param['kode'] = $this->_kode8;
		...
		foreach ($list as $tps) {
			...
			$btngroup = '<div class="input-group">
					...
					<ul class="dropdown-menu">
                        <li>' . anchor("user/aksi/edit/".$index.'/' . base64_encode($tps->rk_kode).'/'. base64_encode($param['id_cip']), "<i class=\"fa fa-edit\"></i>Edit") . '</li>
                        <li>' . anchor("user/aksi/hapus/".$index.'/' . base64_encode($tps->rk_kode).'/'. base64_encode($param['id_cip']), "<i class=\"fa fa-trash\"></i>Hapus") . '</li>
					</ul>
                </div>';
