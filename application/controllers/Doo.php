...
class Doo extends User_Controller
{
    ...
    public function simpan($kode,$id_cip){
        ...
        if ($q){
			...
			redirect('user/doo/edit/'.base64_encode($kode).'/'.base64_encode($id_cip));
	...
	public function hapuslampiran($kode,$id_cip){
		...
		if ($this->m_risalah->delete($param)){
			...
			if ($kode=="OV4"){ // KodeStruktur Organisasi
				redirect('user/doo/upload/strukturorganisasi/'.base64_encode($kode).'/'.base64_encode($id_cip));	
			}else{
				redirect('user/doo/upload/jadwalkegiatan/'.base64_encode($kode).'/'.base64_encode($id_cip));
			}
		}
	}
	...
	public function strukturorganisasi($kode,$id_cip){
		...
		$this->content 		= "doo/uploadstruktur";
        $this->assets 		= array('assets_form');
	...
	public function jadwalkegiatan($kode,$id_cip){
		...
		$this->content 		= "doo/jadwalkegiatan";
        $this->assets 		= array('assets_form');
	...
	{
        if($index==5) $param['kode'] = $this->_kode5;
		...
		foreach ($list as $tps) {
			...
			$btngroup = '<div class="input-group">
					...
					<ul class="dropdown-menu">
                        <li>' . anchor("user/doo/edit/".$index.'/' . base64_encode($tps->rk_kode).'/'. base64_encode($param['id_cip']), "<i class=\"fa fa-edit\"></i>Edit") . '</li>
                        <li>' . anchor("user/doo/hapus/".$index.'/' . base64_encode($tps->rk_kode).'/'. base64_encode($param['id_cip']), "<i class=\"fa fa-trash\"></i>Hapus") . '</li>
					</ul>
                </div>';
